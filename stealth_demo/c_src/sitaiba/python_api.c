...
int sitaiba_addr_verify_simple(unsigned char* addr_buf, unsigned char* r1_buf, unsigned char* r2_buf,
    ...
    int result = sitaiba_addr_verify(Addr, R1, R2, A_r, B_r, A_m, a_r);
...
int sitaiba_addr_verify_fast_simple(unsigned char* r1_buf, unsigned char* r2_buf,
    ...
    int result = sitaiba_addr_verify_fast(R1, R2, A_r, a_r);
...
void sitaiba_performance_test_simple(int iterations, double* results) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        sitaiba_addr_verify(Addr, R1, R2, A_r, B_r, A_m, a_r);
        
        // Test fast verification  
        sitaiba_addr_verify_fast(R1, R2, A_r, a_r);
    ...
    // Set operation count manually since core functions don't auto-increment
