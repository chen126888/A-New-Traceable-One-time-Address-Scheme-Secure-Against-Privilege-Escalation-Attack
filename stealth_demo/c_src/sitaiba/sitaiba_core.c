...
int sitaiba_addr_verify(element_t Addr, element_t R1, element_t R2,
...
int sitaiba_addr_verify_fast(element_t R1, element_t R2, element_t A_r, element_t a_r) {
    clock_t t1 = clock();
...
void sitaiba_get_performance(sitaiba_performance_t* perf) {
    if (!perf || operation_count == 0) return;

    perf->addr_gen_avg = sumAddrGen / operation_count;
    perf->addr_verify_avg = sumAddrVerify / operation_count;
    perf->fast_verify_avg = sumFastAddrVerify / operation_count;
    perf->onetime_sk_avg = sumOnetimeSK / operation_count;
    perf->trace_avg = sumTrace / operation_count;
    perf->h1_avg = sumH1 / operation_count; // For reference
    perf->h2_avg = sumH2 / operation_count; // For reference
    perf->operation_count = operation_count;
}
...
void sitaiba_print_performance(void) {
    ...
    printf("Address Generation:    %.3f ms\n", sumAddrGen / operation_count);
    printf("Address Verify:        %.3f ms\n", sumAddrVerify / operation_count);
    printf("Fast Address Verify:   %.3f ms\n", sumFastAddrVerify / operation_count);
    printf("One-time SK Gen:       %.3f ms\n", sumOnetimeSK / operation_count);
    printf("Identity Tracing:      %.3f ms\n", sumTrace / operation_count);
    printf("\n=== Hash Function Performance (included for reference) ===\n");
    printf("H1 (G1->Zr):          %.3f ms\n", sumH1 / operation_count);
    printf("H2 (GT->Zr):          %.3f ms\n", sumH2 / operation_count);
}
