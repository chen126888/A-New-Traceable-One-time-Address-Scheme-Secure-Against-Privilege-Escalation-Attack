...
import traceback
# Removed sys.path modification as it's no longer needed with proper package structure

# Import scheme services directly
...
class SchemeManager:
    ...
    def get_scheme_capabilities(self, scheme_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        capabilities = {
            ...
            "has_signing": hasattr(service, 'sign_message') and not isinstance(service.sign_message, type(NotImplementedError)), # Check if overridden
            "has_verification": hasattr(service, 'verify_signature') and not isinstance(service.verify_signature, type(NotImplementedError)), # Check if overridden
    ...
    def sign_message(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        service = self.get_current_service()
        # The service method itself will raise NotImplementedError if not supported
