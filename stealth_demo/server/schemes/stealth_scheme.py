...
class StealthScheme(BaseScheme):
    ...
    def verify_signature(self, **kwargs) -> Dict[str, Any]:
        """Verify signature. Supports both sig_id lookup and individual components."""
        if not self.initialized or not self._library:
            raise Exception("Scheme not initialized")
        
        # Check if this is the old sig_id method or new individual components method
        if 'sig_id' in kwargs:
            # Legacy method using sig_id
            sig_id = kwargs['sig_id']
            ...
            is_valid = self._library.verify(addr_bytes, r2_bytes, c_bytes, message, h_bytes, q_sigma_bytes)
        ...
        elif all(key in kwargs for key in ['message', 'q_sigma_hex', 'h_hex', 'addr_id']):
            # New method using individual components from frontend (legacy addr_id method)
            message = kwargs['message']
            q_sigma_hex = kwargs['q_sigma_hex']
            h_hex = kwargs['h_hex']
            ...
            r2_bytes = hex_to_bytes_safe_stealth(addr_info["R2"])
            c_bytes = hex_to_bytes_safe_stealth(addr_info["C"])
            h_bytes = hex_to_bytes_safe_stealth(h_hex)
            q_sigma_bytes = hex_to_bytes_safe_stealth(q_sigma_hex)
        ...
        elif all(key in kwargs for key in ['message', 'q_sigma_hex', 'h_hex', 'addr_hex', 'r2_hex', 'c_hex']):
            # Direct transaction data method - no need to lookup addresses
            message = kwargs['message']
            q_sigma_hex = kwargs['q_sigma_hex']
            h_hex = kwargs['h_hex']
            addr_hex = kwargs['addr_hex']
            r2_hex = kwargs['r2_hex']
            c_hex = kwargs['c_hex']
            
            # Convert hex to bytes directly
            addr_bytes = hex_to_bytes_safe_stealth(addr_hex)
            r2_bytes = hex_to_bytes_safe_stealth(r2_hex)
            c_bytes = hex_to_bytes_safe_stealth(c_hex)
            h_bytes = hex_to_bytes_safe_stealth(h_hex)
            q_sigma_bytes = hex_to_bytes_safe_stealth(q_sigma_hex)
            ...
            raise ValueError("Either provide sig_id OR (message, q_sigma_hex, h_hex, addr_id) OR (message, q_sigma_hex, h_hex, addr_hex, r2_hex, c_hex) parameters")
    ...
    def get_status(self) -> Dict[str, Any]:
        """Get current stealth scheme status."""
        status = {
            "scheme_name": self.info.name,
            "scheme_version": self.info.version,
    ...
    def _normalize_keys(self, raw_keys: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Normalize stealth keys to match frontend expected format (same as sitaiba)."""
        normalized = []
        for key in raw_keys:
            # Extract from nested structure if needed
            if 'public_key_A' in key:  # New keygen format
                normalized.append({
                    "id": f"stealth_key_{key['key_id']}",
                    "index": key["key_id"], 
                    "scheme": "stealth",
                    "A_hex": key["public_key_A"]["hex"],     # Match sitaiba format
                    "B_hex": key["public_key_B"]["hex"],     # Match sitaiba format
                    "a_hex": key["private_key_a"]["hex"],    # Match sitaiba format
                    "b_hex": key["private_key_b"]["hex"],    # Match sitaiba format
                    "created_at": key.get("created_at"),
                    "param_file": self.current_param_file,
                    "status": "generated"
                })
            else:  # Direct format (from keylist)
                normalized.append({
                    "id": f"stealth_key_{key.get('key_id', key.get('index', 0))}",
                    "index": key.get("key_id", key.get("index", 0)),
                    "scheme": "stealth", 
                    "A_hex": key.get("A", ""),               # Match sitaiba format
                    "B_hex": key.get("B", ""),               # Match sitaiba format
                    "a_hex": key.get("a", ""),               # Match sitaiba format
                    "b_hex": key.get("b", ""),               # Match sitaiba format
                    "created_at": key.get("created_at"),
                    "param_file": self.current_param_file,
